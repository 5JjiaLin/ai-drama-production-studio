...
import { generateStoryboardInChunks } from './gemini-chunking';
import { getApiConfig } from './apiConfig';


// Helper functions to get current API config
const getApiKey = () => getApiConfig().apiKey;
const getBaseUrl = () => getApiConfig().baseUrl;

// Lazy initialize Gemini Client (only when needed)
const getGeminiClient = () => {
  return new GoogleGenAI({
    apiKey: getApiKey() || 'placeholder',
...
const callDeepSeek = async (model: string, prompt: string, systemInstruction: string): Promise<string> => {
   ...
   const timeoutId = setTimeout(() => controller.abort(), 900000); // 增加到15分钟超时

   try {
     // 判断是否是官方 DeepSeek API
     const isDeepSeekOfficial = baseUrl.includes('deepseek.com');
     const endpoint = isDeepSeekOfficial ? `${baseUrl}/v1/chat/completions` : `${baseUrl}/chat/completions`;
    ...
    return json.choices[0].message.content;
     ...
     if (error.name === 'AbortError') {
       throw new Error('请求超时（5分钟），请尝试减少分镜数量或缩短剧本长度');
     }
     throw error;
   }
};

// Helper: Call Claude API (using unified API)
const callClaude = async (model: string, prompt: string, systemInstruction: string, timeoutMs: number = 600000): Promise<string> => {
   const apiKey = getApiKey();
   const baseUrl = getBaseUrl();

   console.log('[Debug] callClaude - API Key存在:', !!apiKey, 'Base URL:', baseUrl);

   if (!apiKey) {
     throw new Error("请先在页面顶部输入API Key");
   }

   // 清理API Key，去除所有非ASCII字符和空白字符
   const cleanApiKey = apiKey.replace(/[^\x00-\x7F]/g, '').trim();

   if (!cleanApiKey) {
     throw new Error("API Key格式错误：包含非法字符，请检查并重新输入");
   }

   console.log('[Debug] API Key清理后长度:', cleanApiKey.length);
   ...
   try {
     // 判断是否是第三方 API (代理模式)
     const isThirdPartyAPI = baseUrl === '/api' || baseUrl.includes('backgrace');

     if (isThirdPartyAPI) {
       // 第三方 API 使用 OpenAI 兼容格式
       console.log('[Debug] 使用第三方 API OpenAI 兼容格式');
       ...
       if (!response.ok) {
         const errText = await response.text();
         console.error('[Debug] 错误响应:', errText);
         throw new Error(`API 请求失败 (${response.status}): ${errText}`);
       }
       ...
       console.log('[Debug] 发送请求到:', `${baseUrl}/v1/messages`);
       const response = await fetch(`${baseUrl}/v1/messages`, {
         ...
         body: JSON.stringify({
           model: model,
           max_tokens: 65536, // 大幅提高输出token限制，支持更多分镜
     ...
     clearTimeout(timeoutId);
     console.error('[Debug] callClaude请求失败:', error);

     if (error.name === 'AbortError') {
       throw new Error(`请求超时（${Math.floor(timeoutMs/60000)}分钟），请尝试使用分段生成模式`);
     }

     if (error.message.includes('Failed to fetch') || error.name === 'TypeError') {
       throw new Error('网络连接失败：可能是CORS问题、网络不稳定或API服务器不可用。建议：1) 检查网络连接 2) 确认API服务器地址正确 3) 联系API服务提供商确认浏览器访问权限');
     }
...
// Helper: Call OpenAI-compatible API (GPT models)
const callOpenAI = async (model: string, prompt: string, systemInstruction: string, timeoutMs: number = 600000): Promise<string> => {
   const apiKey = getApiKey();
   const baseUrl = getBaseUrl();
   ...
   const controller = new AbortController();
   const timeoutId = setTimeout(() => controller.abort(), timeoutMs); // Configurable timeout (default 10 minutes)
...
// Helper: Call Gemini API (using unified API or native SDK)
const callGemini = async (model: string, prompt: string, systemInstruction: string, timeoutMs: number = 600000): Promise<string> => {
   const apiKey = getApiKey();
   const baseUrl = getBaseUrl();
...
export const analyzeScriptDeeply = async (
  ...
  const maxCharLimit = (isDeepSeek || isClaude || isOpenAI) ? 500000 : 1000000; // 大幅提高上下文长度
  ...
  if (isDeepSeek) {
    ...
    try {
      ...
      console.log('[Debug] DeepSeek 返回内容:', resultText.substring(0, 200));
      const cleaned = cleanJsonOutput(resultText);
      console.log('[Debug] 清理后的 JSON:', cleaned.substring(0, 200));
      return cleanContent(JSON.parse(cleaned)) as ScriptAnalysis;
    } catch (error: any) {
      console.error('[Error] JSON 解析失败:', error.message);
      throw new Error(`剧本分析失败: ${error.message}。请检查 API 返回内容格式。`);
    }
  }
  ...
  if (isClaude) {
    ...
    try {
      ...
      console.log('[Debug] Claude 返回内容:', resultText.substring(0, 200));
      const cleaned = cleanJsonOutput(resultText);
      console.log('[Debug] 清理后的 JSON:', cleaned.substring(0, 200));
      return cleanContent(JSON.parse(cleaned)) as ScriptAnalysis;
    } catch (error: any) {
  ...
  if (isOpenAI) {
    const systemInstruction = "You are a script analysis expert. Please output valid JSON only.";
    try {
      const resultText = await callOpenAI(modelName, prompt, systemInstruction);
      console.log('[Debug] OpenAI 返回内容:', resultText.substring(0, 200));
      const cleaned = cleanJsonOutput(resultText);
      console.log('[Debug] 清理后的 JSON:', cleaned.substring(0, 200));
      return cleanContent(JSON.parse(cleaned)) as ScriptAnalysis;
    } catch (error: any) {
...
export const generateStoryboard = async (
  ...
  modelName: string = "gemini-2.5-flash",
  analysisContext?: ScriptAnalysis | null,
  onProgress?: (progress: { current: number; total: number; message: string }) => void // 新增：进度回调
  ...
  // 分段模式：大幅提高分段阈值，支持一次性生成更多分镜
  // 只有在分镜数超过50个时才启用分段
  const isGemini = !isDeepSeek && !isClaude && !isOpenAI;
  const shouldChunk = !feedback && maxShots && maxShots > 50 && (isClaude || isOpenAI || isGemini || isDeepSeek);

  if (shouldChunk) {
    // 使用分段生成模式
    return await generateStoryboardInChunks(
      scriptText,
      basicElements || null,
      minShots || 10,
      maxShots,
      ...
      callOpenAI,
      onProgress,
      callGemini,  // 传递 callGemini 函数
      callDeepSeek  // 传递 callDeepSeek 函数
    );
  }
